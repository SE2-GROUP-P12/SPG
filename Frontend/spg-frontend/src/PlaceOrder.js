...
function PlaceOrder(props) {
    ...
    /***************************************************************************/
    if (props.loggedUserRole === 'EMPLOYEE' || props.loggedUserRole === 'ADMIN') {
        email = props.loggedUser;
    }
    else if (props.loggedUserRole === 'CUSTOMER') {
        setCustomer(props.loggedUser);
        setCustomerSuccess(true);
    }
    
    useEffect(() => {
        let getCartInfo = async (customerEmail) => {
            let prod = await API.getCart({ 'email': customerEmail }, props.setErrorMessage);
            if (prod === null) {
    ...
    return (
        <Container >
            <h1 style={{ paddingBlock: "20px" }}>Place Order</h1>
            {itsTime ? null :
                <Alert xs variant='warning'> It's possible to place orders only from Saturday at 9am to Sunday at
                    11pm</Alert>}
            ...
            <Row>
                {props.loggedUserRole !== 'CUSTOMER' ?
                    <Col>
                        <Button variant='secondary' href="/ShopEmployee">Back</Button>
                    </Col>
                    :
                    <Col>
                        <Button variant='secondary' href="/Customer">Back</Button>
                    </Col>
                }
